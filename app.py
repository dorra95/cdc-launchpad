...
def _pdf_cover_page(
    ...
    kind_label = {
        ...
        "portfolio": ("Rapport portefeuille CDC" if is_fr else "CDC Portfolio Report"),
...
def _matplotlib_top_chart(
    series: pd.Series, title: str, color_hex: str = NAVY, max_items: int = 12
) -> io.BytesIO | None:
    """Render a horizontal bar chart as a high-res PNG ready to embed in a PDF."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        ...
        return None
    s = series.head(max_items).sort_values()
    fig, ax = plt.subplots(figsize=(6.6, max(2.5, 0.32 * len(s))), dpi=170)
    ax.barh(s.index.astype(str), s.values, color=color_hex, height=0.7)
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.tick_params(left=False, labelsize=9, colors="#374151")
    ax.set_xlabel("")
    ax.set_title(title, color=NAVY, fontsize=11, weight="bold", pad=10, loc="left")
    ax.set_axisbelow(True)
    ax.grid(axis="x", linestyle="--", alpha=0.30, color="#D1D5DB")
    for i, v in enumerate(s.values):
        ax.text(v, i, f"  {int(v):,}", va="center", fontsize=9, color=NAVY)
    fig.tight_layout()
    buf = io.BytesIO()
...
def _matplotlib_yearly_line(years: pd.Series) -> io.BytesIO | None:
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    if yearly.empty:
        return None
    fig, ax = plt.subplots(figsize=(6.6, 2.6), dpi=170)
    ax.fill_between(yearly.index.astype(int), yearly.values, color=NAVY, alpha=0.18)
    ax.plot(yearly.index.astype(int), yearly.values, color=RED, lw=2.2, marker="o", markersize=4)
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    ax.tick_params(labelsize=9, colors="#374151")
    ax.set_axisbelow(True)
    ax.grid(axis="y", linestyle="--", alpha=0.30, color="#D1D5DB")
    fig.tight_layout()
    buf = io.BytesIO()
    fig.savefig(buf, format="png", bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def portfolio_pdf(
    df: pd.DataFrame, summary: pd.DataFrame, lang: str = "FR"
) -> io.BytesIO:
    """Detailed portfolio report: cover page, KPI headline, embedded charts
    (sector top, region top, founding-year cadence), portfolio segments,
    beneficiary spotlight, and programme references."""
    from reportlab.lib import colors
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.styles import ParagraphStyle, getSampleStyleSheet
    from reportlab.lib.units import mm
    ...
    is_fr = (lang == "FR")
    L = {
        "title": "Rapport portefeuille CDC" if is_fr else "CDC portfolio report",
        "headline": "Indicateurs cles" if is_fr else "Headline indicators",
        "sector_chart": "Top secteurs - portefeuille CDC" if is_fr else "Top sectors - CDC portfolio",
        "region_chart": "Repartition par gouvernorat" if is_fr else "Distribution by governorate",
        "year_chart": "Cohortes annuelles" if is_fr else "Yearly cohorts",
        "segments": "Segments du portefeuille" if is_fr else "Portfolio segments",
        "spotlight": "Coup de projecteur beneficiaires" if is_fr else "Beneficiary spotlight",
        "programs": "Programmes CDC actifs" if is_fr else "Active CDC programmes",
        "footer": ("Document genere automatiquement par CDC LAUNCHPAD."
                   if is_fr else "Auto-generated by CDC LAUNCHPAD."),
    }
    ...
    styles = getSampleStyleSheet()
    title = ParagraphStyle("PTitle", parent=styles["Title"], textColor=colors.HexColor(NAVY))
    h2 = ParagraphStyle("PH2", parent=styles["Heading2"],
                        textColor=colors.HexColor(NAVY), spaceBefore=4, spaceAfter=4)
    body = styles["BodyText"]
    small = ParagraphStyle("PSm", parent=body, fontSize=8, textColor=colors.HexColor(MUTED))
    elements: list[Any] = []

    # Cover page (uses the same branded helper as the other reports)
    funded_total = int(df.get("funded", pd.Series([0])).sum())
    elements.extend(_pdf_cover_page(
        {"name": "CDC Tunisie", "sector": "Portfolio", "region": "Tunisia"},
        kind="portfolio",
        badge_text=(f"{len(df):,} startups suivies" if is_fr else f"{len(df):,} startups tracked"),
        badge_color=NAVY,
        subtitle=(f"{funded_total:,} beneficiaires actifs"
                  if is_fr else f"{funded_total:,} active beneficiaries"),
        lang=lang,
    ))

    # Headline KPI strip
    elements.append(Paragraph(L["headline"], h2))
    kpi_rows = [[
        Paragraph(f"<b>{len(df):,}</b><br/>"
                  f"<font size=8 color='#6B7280'>"
                  f"{'Startups dans la base' if is_fr else 'Startups in base'}"
                  f"</font>", body),
        ...
        Paragraph(f"<b>{df.get('funded', pd.Series([0])).mean() * 100:.1f}%</b><br/>"
                  f"<font size=8 color='#6B7280'>"
                  f"{'Taux de financement' if is_fr else 'Funding rate'}"
                  f"</font>", body),
        Paragraph(f"<b>{int(df['sector'].nunique()):,}</b><br/>"
                  f"<font size=8 color='#6B7280'>"
                  f"{'Secteurs distincts' if is_fr else 'Distinct sectors'}"
                  f"</font>", body),
    ]]
    kt = Table(kpi_rows, colWidths=[44 * mm, 44 * mm, 44 * mm, 44 * mm], rowHeights=[18 * mm])
    kt.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#F8F9FC")),
        ("BOX", (0, 0), (-1, -1), 0.4, colors.HexColor("#EEF0F6")),
        ("LINEAFTER", (0, 0), (-2, 0), 0.4, colors.HexColor("#EEF0F6")),
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ("ALIGN", (0, 0), (-1, -1), "LEFT"),
    ...
    # Sector chart
    sector_buf = _matplotlib_top_chart(
        df["sector"].value_counts().head(12),
        L["sector_chart"], color_hex=NAVY,
    )
    if sector_buf is not None:
        elements.append(Image(sector_buf, width=170 * mm, height=80 * mm))
        elements.append(Spacer(1, 6))

    # Regional chart
    if "Region" in df.columns:
        region_buf = _matplotlib_top_chart(
            df["Region"].value_counts().head(12),
            L["region_chart"], color_hex=RED,
        )
        if region_buf is not None:
            elements.append(Image(region_buf, width=170 * mm, height=80 * mm))
            elements.append(Spacer(1, 6))

    # Yearly cadence
    if "founding_year" in df.columns:
        years_clean = pd.to_numeric(df["founding_year"], errors="coerce").dropna().astype(int)
        year_buf = _matplotlib_yearly_line(years_clean)
        if year_buf is not None:
            elements.append(Paragraph(L["year_chart"], h2))
            elements.append(Image(year_buf, width=170 * mm, height=58 * mm))
            elements.append(Spacer(1, 8))

    # Portfolio segments table
    if not summary.empty:
        elements.append(Paragraph(L["segments"], h2))
        seg_header = (("Segment", "Startups", "% finances", "Age moyen")
                      if is_fr else ("Segment", "Startups", "Funded %", "Avg age"))
        seg_rows = [list(seg_header)]
        for _, row in summary.iterrows():
            seg_rows.append([
                str(row.get("profile", "")),
                f"{int(row.get('startups', 0)):,}",
                f"{float(row.get('funded_rate', 0)):.1f}%",
                f"{float(row.get('avg_age', 0)):.1f}",
            ])
        seg_table = Table(
            seg_rows,
            colWidths=[72 * mm, 30 * mm, 30 * mm, 30 * mm],
        )
    ...
    # Beneficiary spotlight - 4 funded startups with the richest profile
    elements.append(PageBreak())
    elements.append(Paragraph(L["spotlight"], title))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        ("Les startups financees qui rythment le portefeuille CDC."
         if is_fr else
         "Funded teams driving the CDC portfolio."), small))
    elements.append(Spacer(1, 8))

    spotlights = beneficiary_spotlight(df, k=4)
    for sp in spotlights:
        labelled_chip = ("Startup Act" if sp.get("labelled") else
                         ("Non labellisee" if is_fr else "Unlabelled"))
        block = [
            Paragraph(f"<font color='{RED}' size=9><b>{sp['sector'][:36]}</b></font> "
                      f"&nbsp; <font size=8 color='#6B7280'>{sp['region'][:30]}</font>", body),
            Paragraph(f"<font color='{NAVY}'><b>{sp['name'][:60]}</b></font>", h2),
            Paragraph(
                ("Fondee en " if is_fr else "Founded in ")
                + (f"{sp['year']}" if sp.get("year") else "n/d")
                + f"  ·  {labelled_chip}",
                small,
            ),
            Spacer(1, 6),
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(L["programs"], h2))
    for prog in CDC_PROGRAMS:
        line = (
            f"<b>{prog['name']}</b> &nbsp; "
            f"<font size=8 color='#6B7280'>{prog['operator']}</font><br/>"
            f"<font size=8>{prog['budget']} &nbsp;·&nbsp; {prog['period']} &nbsp;·&nbsp; "
            f"{prog['stage']}</font><br/>"
            f"<font size=8 color='#374151'>"
            f"{prog['summary_fr'] if is_fr else prog['summary_en']}"
            f"</font>"
        )
...
# ---------------------------------------------------------------------------
# Product-stage guide - TRL bands, valuation method per stage, best practice
# ---------------------------------------------------------------------------
STAGE_GUIDE: list[dict[str, Any]] = [
    {
        "key": "ideation",
        "engine_stage": 0,
        "label_fr": "Ideation (TRL 1-2)",
        "label_en": "Ideation (TRL 1-2)",
        "trl": "TRL 1-2 - principes de base, concept formule",
        "signals_fr": [
            "Hypothese de probleme et solution exprimee",
            "Aucun prototype ou code livre",
            "Equipe en formation, premiers entretiens utilisateurs",
        ],
        "signals_en": [
            "Problem and solution hypothesis articulated",
            "No prototype or code shipped yet",
            "Team forming, first user interviews",
        ],
        "best_method": "Berkus",
        "method_reason_fr": "Aucun revenu ni prototype - on derisque idee, equipe et opportunite (5 facteurs Berkus).",
        "method_reason_en": "No revenue and no prototype - de-risk by idea / team / opportunity (5 Berkus factors).",
        "founder_fr": [
            "Documenter 20+ entretiens utilisateurs cibles",
            "Formaliser un Problem Statement et une hypothese de valeur",
            "Definir une roadmap d'experimentation 0-90 jours",
        ],
        "founder_en": [
            "Document 20+ target-user interviews",
            "Write a problem statement + value hypothesis",
            "Build a 0-90 day experimentation roadmap",
        ],
        "evaluator_fr": [
            "Verifier la qualite des entretiens (signal vs bruit)",
            "Tester la differenciation et la taille du marche adressable",
            "Evaluer le caliber et la complementarite de l'equipe",
        ],
        "evaluator_en": [
            "Audit interview quality (signal vs noise)",
            "Stress-test differentiation and addressable market",
            "Assess team caliber and complementarity",
        ],
        "ticket_fr": "Subvention 10-50 k TND - amorcage ideation",
        "ticket_en": "Grant 10-50K TND - ideation jumpstart",
    },
    {
        "key": "proto",
        "engine_stage": 0,
        "label_fr": "Prototype / PoC (TRL 3-4)",
        "label_en": "Prototype / PoC (TRL 3-4)",
        "trl": "TRL 3-4 - preuve de concept en environnement de laboratoire",
        "signals_fr": [
            "Prototype technique fonctionnel, non encore deploye",
            "Premieres validations utilisateurs (tests qualitatifs)",
            "Pas encore de revenus reguliers",
        ],
        "signals_en": [
            "Working technical prototype, not deployed",
            "First qualitative user validations",
            "No recurring revenue yet",
        ],
        "best_method": "Berkus + Scorecard",
        "method_reason_fr": "Prototype existe mais pas de marche - Berkus capture le derisque, Scorecard pondere les 7 axes vs baseline.",
        "method_reason_en": "Prototype exists but no market - Berkus captures the de-risk, Scorecard weights 7 axes vs baseline.",
        "founder_fr": [
            "Livrer un PoC mesurable en 6-12 semaines",
            "Documenter 3-5 KPIs techniques (uptime, latence, taux d'erreur)",
            "Securiser 1 partenaire pilote de reference",
        ],
        "founder_en": [
            "Ship a measurable PoC in 6-12 weeks",
            "Document 3-5 technical KPIs (uptime, latency, error rate)",
            "Secure 1 pilot partner of reference",
        ],
        "evaluator_fr": [
            "Inspecter le code / la stack et la dette technique",
            "Verifier la propriete intellectuelle et l'inventeur",
            "Cadrer le plan de transition vers MVP commercial",
        ],
        "evaluator_en": [
            "Inspect the stack and technical debt",
            "Verify IP ownership and inventor track",
            "Frame the path to commercial MVP",
        ],
        "ticket_fr": "Avance remboursable 50-200 k TND - finalisation PoC (VAIR)",
        "ticket_en": "Repayable advance 50-200K TND - PoC finalisation (VAIR)",
    },
    {
        "key": "mvp",
        "engine_stage": 1,
        "label_fr": "MVP / Beta (TRL 5-6)",
        "label_en": "MVP / Beta (TRL 5-6)",
        "trl": "TRL 5-6 - prototype valide en environnement representatif",
        "signals_fr": [
            "MVP en production, premiers utilisateurs actifs",
            "Retention initiale mesurable (D7, D30)",
            "Pricing en cours de test, revenus < 100 k TND/an",
        ],
        "signals_en": [
            "MVP in production, first active users",
            "Initial retention metrics (D7, D30)",
            "Pricing under test, revenue < 100K TND/year",
        ],
        "best_method": "Scorecard + Risk Factor Summation",
        "method_reason_fr": "Marche emergent mais pas encore predictible - Scorecard ancre sur la baseline, RFS isole les 12 risques sectoriels.",
        "method_reason_en": "Emerging market, not yet predictable - Scorecard anchors on baseline, RFS isolates the 12 sector risks.",
        "founder_fr": [
            "Mesurer activation, retention et NPS chaque semaine",
            "Verrouiller 5-10 clients de reference contractuels",
            "Pre-calculer LTV / CAC avant tour suivant",
        ],
        "founder_en": [
            "Weekly tracking of activation, retention and NPS",
            "Lock 5-10 contractual reference customers",
            "Pre-compute LTV / CAC before next round",
        ],
        "evaluator_fr": [
            "Stress-test du product-market signal (cohortes)",
            "Verifier la rentabilite unitaire et le burn",
            "Auditer les processus operationnels (delivery, support)",
        ],
        "evaluator_en": [
            "Stress-test the product-market signal (cohorts)",
            "Check unit economics and burn",
            "Audit operational processes (delivery, support)",
        ],
        "ticket_fr": "Seed 200-800 k TND - convertir MVP en machine commerciale",
        "ticket_en": "Seed 200-800K TND - convert MVP into a sales machine",
    },
    {
        "key": "early_traction",
        "engine_stage": 1,
        "label_fr": "Premiere traction (TRL 7-8)",
        "label_en": "Early traction (TRL 7-8)",
        "trl": "TRL 7-8 - demontre en environnement reel / pre-commercial",
        "signals_fr": [
            "Revenus 100 k - 1 M TND, croissance YoY > 50%",
            "Cycle de vente structure, ARR identifiable",
            "Premier embauche commerciale",
        ],
        "signals_en": [
            "Revenue 100K - 1M TND, YoY growth > 50%",
            "Structured sales cycle, identifiable ARR",
            "First commercial hire",
        ],
        "best_method": "Venture Capital Method + Risk Factor Summation",
        "method_reason_fr": "Revenus mesurables - methode VC projette la sortie, RFS ajuste pour les risques specifiques.",
        "method_reason_en": "Measurable revenue - VC method projects exit, RFS adjusts for specific risks.",
        "founder_fr": [
            "Documenter la repetabilite du cycle de vente",
            "Construire un sales funnel chiffre (top of funnel a closed-won)",
            "Mettre en place un reporting investor-ready mensuel",
        ],
        "founder_en": [
            "Document sales-cycle repeatability",
            "Build a quantified funnel (top of funnel to closed-won)",
            "Set up monthly investor-ready reporting",
        ],
        "evaluator_fr": [
            "Confirmer la qualite des revenus (recurrent vs one-off)",
            "Verifier la sante financiere (cash, runway, dette)",
            "Modeliser 3-5 scenarios de sortie",
        ],
        "evaluator_en": [
            "Confirm revenue quality (recurring vs one-off)",
            "Verify financial health (cash, runway, debt)",
            "Model 3-5 exit scenarios",
        ],
        "ticket_fr": "Serie A 800 k - 3 M TND - acceleration commerciale",
        "ticket_en": "Series A 800K - 3M TND - sales acceleration",
    },
    {
        "key": "scale",
        "engine_stage": 2,
        "label_fr": "Revenus / Scale (TRL 9)",
        "label_en": "Revenue / Scale (TRL 9)",
        "trl": "TRL 9 - systeme commercial deploye, perimetre operationnel",
        "signals_fr": [
            "Revenus > 1 M TND, ARR predictible",
            "Marges unitaires saines, NRR > 100%",
            "Equipe complete, gouvernance formelle",
        ],
        "signals_en": [
            "Revenue > 1M TND, predictable ARR",
            "Healthy unit margins, NRR > 100%",
            "Full team, formal governance",
        ],
        "best_method": "Hybrid DCF + VC Method",
        "method_reason_fr": "Cashflows projetables - DCF ancre sur les EBITDA actualises, methode VC valide la sortie.",
        "method_reason_en": "Projectable cashflows - DCF anchors on discounted EBITDA, VC method validates the exit.",
        "founder_fr": [
            "Industrialiser le commercial (playbook, RevOps)",
            "Securiser le levier de croissance (M&A, partenariats)",
            "Preparer l'audit financier annuel (Big Four ou equivalent)",
        ],
        "founder_en": [
            "Industrialise sales (playbook, RevOps)",
            "Secure the growth lever (M&A, partnerships)",
            "Prep annual audit (Big Four or equivalent)",
        ],
        "evaluator_fr": [
            "Modeliser DCF a 5 ans + valeur terminale",
            "Comparer avec multiples de transactions sectorielles",
            "Verifier la gouvernance et la conformite ESG",
        ],
        "evaluator_en": [
            "Build 5-year DCF + terminal value",
            "Benchmark against sector transaction multiples",
            "Verify governance and ESG compliance",
        ],
        "ticket_fr": "Serie B+ 3-15 M TND - expansion regionale",
        "ticket_en": "Series B+ 3-15M TND - regional expansion",
    },
]
...
def plotly_tunisia_map(
    ...
    """Mapbox bubble map of Tunisia. Dimension controls the bubble color metric:
    'density', 'funded_rate', 'recent', or 'sector_mix'."""
    import plotly.graph_objects as go
    ...
    if not rows:
        return None
    plot_df = pd.DataFrame(rows).sort_values("count", ascending=False).reset_index(drop=True)
    max_c = max(1, plot_df["count"].max())
    # Larger, more visible bubbles - especially for big regions
    plot_df["size"] = 22 + (plot_df["count"] / max_c) * 60

    # Show count INSIDE the top bubbles so the map is informative at a glance.
    plot_df["bubble_label"] = [
        f"{c}" if (i < 8 or c >= max_c * 0.25) else ""
        for i, c in enumerate(plot_df["count"])
    ]

    sector_palette = [NAVY, RED, GOLD, TEAL, VIOLET, ROSE, BLUE, AMBER, GREEN,
                      "#0B5394", "#A64D79", "#7C9100"]
    fig = go.Figure()
    sector_to_color: dict[str, str] = {}

    if dimension == "sector_mix":
        unique_sectors = list(dict.fromkeys(s for s in plot_df["top_sector"] if s))
        ...
        marker_dict = dict(
            ...
            colorscale=[
                [0.0, "#9BB1E0"], [0.25, "#5F73B3"],
                [0.55, NAVY], [0.80, "#7E1620"], [1.0, RED],
            ],
    ...
    # Glow halo behind bubbles for visual lift on the basemap
    fig.add_trace(go.Scattermapbox(
        lon=plot_df["lon"], lat=plot_df["lat"], mode="markers",
        marker=dict(size=plot_df["size"] * 1.55,
                    color="rgba(209,10,17,0.10)"),
        hoverinfo="skip", showlegend=False,
    ))
    # Main bubble layer with in-bubble count labels for top regions
    fig.add_trace(go.Scattermapbox(
        lon=plot_df["lon"], lat=plot_df["lat"],
        mode="markers+text",
        text=plot_df["bubble_label"],
        textposition="middle center",
        textfont=dict(size=13, color="white", family="Inter, sans-serif"),
        marker=marker_dict,
        customdata=plot_df[["region", "count", "funded",
                            "funded_rate", "avg_year", "top_sector"]].values,
        hovertemplate=(
            "<b>%{customdata[0]}</b><br>"
            "Startups : %{customdata[1]}<br>"
            "Finances : %{customdata[2]} (%{customdata[3]:.0f}%)<br>"
            "Annee moyenne : %{customdata[4]:.0f}<br>"
            "Top secteur : %{customdata[5]}<extra></extra>"
        ),
        showlegend=False,
    ))
    # Region-name labels floating above each bubble for clarity
    fig.add_trace(go.Scattermapbox(
        lon=plot_df["lon"], lat=plot_df["lat"],
        mode="text",
        text=plot_df["region"],
        textposition="top center",
        textfont=dict(size=11, color=NAVY, family="Inter, sans-serif"),
        hoverinfo="skip", showlegend=False,
    ))
    ...
    if dimension == "sector_mix":
        ...
        if not top3.empty:
            fig.add_trace(go.Scattermapbox(
                lon=top3["lon"], lat=top3["lat"], mode="markers",
                marker=dict(size=top3["size"] * 1.95,
                            color="rgba(209,10,17,0.0)",
                            ),
                hoverinfo="skip", showlegend=False,
            ))
        ...
        fig.update_layout(legend=dict(
            font=dict(size=10, color=NAVY), bgcolor="rgba(255,255,255,0.92)",
            bordercolor="#EEF0F6", borderwidth=1,
            x=1.02, y=0.5, yanchor="middle",
        ))

    fig.update_layout(
        mapbox=dict(
            style="carto-positron",  # No token required, clean cartographic base
            center=dict(lat=34.4, lon=9.6),
            zoom=5.4,
        ),
    )
...
def run_app() -> None:
    ...
    with inner_tabs[0]:
        ...
        with st.form("assessment_form"):
            ...
            stage_options = [s["key"] for s in STAGE_GUIDE]
            stage_format = {
                s["key"]: (s["label_en"] if lang == "EN" else s["label_fr"])
                for s in STAGE_GUIDE
            }
            stage_key = c6.selectbox(
                stage_label, stage_options,
                index=2,  # default to MVP
                format_func=lambda k: stage_format.get(k, k),
            )
            session["stage_key"] = stage_key
            # Legacy engine input ("Idea"/"MVP"/"Revenue-generating") derived
            # from the engine_stage int on each entry.
            stage = ["Idea", "MVP", "Revenue-generating"][stage_by_key(stage_key)["engine_stage"]]
        ...
        # --- Stage guide: TRL, valuation method, best practices ----------
        active_key = session.get("stage_key", "mvp")
        stage_info = stage_by_key(active_key)
        is_fr_sg = (lang == "FR")
        sig = stage_info["signals_fr"] if is_fr_sg else stage_info["signals_en"]
        fo = stage_info["founder_fr"] if is_fr_sg else stage_info["founder_en"]
        ev = stage_info["evaluator_fr"] if is_fr_sg else stage_info["evaluator_en"]
        mreason = stage_info["method_reason_fr"] if is_fr_sg else stage_info["method_reason_en"]
        ticket = stage_info["ticket_fr"] if is_fr_sg else stage_info["ticket_en"]
        guide_title = (
            f"Stage guide - {stage_info['label_fr'] if is_fr_sg else stage_info['label_en']}"
        )
        with st.expander(guide_title, expanded=False):
            top = st.columns([1, 1])
            with top[0]:
                st.markdown(
                    f"**TRL**  \n{stage_info['trl']}"
                ...
                st.markdown(
                    "**" + ("Signaux" if is_fr_sg else "Signals") + "**"
                )
                for line in sig:
                    st.markdown(f"- {line}")
            with top[1]:
                st.markdown(
                    f"**" + ("Methode de valorisation" if is_fr_sg else "Valuation method") + f"**  \n"
                    f"<span style='display:inline-block;background:linear-gradient(135deg,"
                    f"{NAVY},{RED});color:white;padding:0.25rem 0.75rem;border-radius:999px;"
                    f"font-weight:700;font-size:0.82rem;letter-spacing:.3px'>"
                    f"{stage_info['best_method']}</span>",
                    unsafe_allow_html=True,
                )
                st.caption(mreason)
                st.markdown(
                    "**" + ("Ticket indicatif" if is_fr_sg else "Indicative ticket") + "**  \n"
                    + ticket
                )
            st.divider()
            bot = st.columns([1, 1])
            with bot[0]:
                st.markdown(
                    "**" + ("Bonnes pratiques fondateur" if is_fr_sg else "Founder best practice") + "**"
                )
                for line in fo:
                    st.markdown(f"- {line}")
            with bot[1]:
                st.markdown(
                    "**" + ("Bonnes pratiques evaluateur" if is_fr_sg else "Evaluator best practice") + "**"
                )
                for line in ev:
                    st.markdown(f"- {line}")
    ...
    with tabs[5]:
        ...
        with p1:
            st.download_button(
                "Portefeuille - PDF",
                portfolio_pdf(df, segment_summary, lang=lang),
